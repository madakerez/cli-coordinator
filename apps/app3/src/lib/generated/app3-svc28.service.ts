...
export interface App3Svc28ServiceConfig {
  baseUrl: string;
  apiKey: string;
  timeout: number;
...
export interface App3Svc28CacheEntry<T> {
  data: T;
  timestamp: number;
  ttl: number;
  key: string;
...
export class App3Svc28Service {
  private cache = new Map<string, App3Svc28CacheEntry<unknown>>();
  private requestQueue: Array<() => Promise<void>> = [];
  private processing = false;

  constructor(private config: App3Svc28ServiceConfig) {}
