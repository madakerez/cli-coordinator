...
export interface App3Svc30ServiceConfig {
  baseUrl: string;
  apiKey: string;
  timeout: number;
...
export interface App3Svc30CacheEntry<T> {
  data: T;
  timestamp: number;
  ttl: number;
  key: string;
...
export class App3Svc30Service {
  private cache = new Map<string, App3Svc30CacheEntry<unknown>>();
  private requestQueue: Array<() => Promise<void>> = [];
  private processing = false;

  constructor(private config: App3Svc30ServiceConfig) {}
