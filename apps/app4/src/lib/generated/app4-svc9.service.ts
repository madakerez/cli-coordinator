...
export interface App4Svc9ServiceConfig {
  baseUrl: string;
  apiKey: string;
  timeout: number;
...
export interface App4Svc9CacheEntry<T> {
  data: T;
  timestamp: number;
  ttl: number;
  key: string;
...
export class App4Svc9Service {
  private cache = new Map<string, App4Svc9CacheEntry<unknown>>();
  private requestQueue: Array<() => Promise<void>> = [];
  private processing = false;

  constructor(private config: App4Svc9ServiceConfig) {}
