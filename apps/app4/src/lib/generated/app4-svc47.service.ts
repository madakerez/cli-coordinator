...
export interface App4Svc47ServiceConfig {
  baseUrl: string;
  apiKey: string;
  timeout: number;
...
export interface App4Svc47CacheEntry<T> {
  data: T;
  timestamp: number;
  ttl: number;
  key: string;
...
export class App4Svc47Service {
  private cache = new Map<string, App4Svc47CacheEntry<unknown>>();
  private requestQueue: Array<() => Promise<void>> = [];
  private processing = false;

  constructor(private config: App4Svc47ServiceConfig) {}
