...
export interface App4Svc25ServiceConfig {
  baseUrl: string;
  apiKey: string;
  timeout: number;
...
export interface App4Svc25CacheEntry<T> {
  data: T;
  timestamp: number;
  ttl: number;
  key: string;
...
export class App4Svc25Service {
  private cache = new Map<string, App4Svc25CacheEntry<unknown>>();
  private requestQueue: Array<() => Promise<void>> = [];
  private processing = false;

  constructor(private config: App4Svc25ServiceConfig) {}
