...
export interface App4Svc20ServiceConfig {
  baseUrl: string;
  apiKey: string;
  timeout: number;
...
export interface App4Svc20CacheEntry<T> {
  data: T;
  timestamp: number;
  ttl: number;
  key: string;
...
export class App4Svc20Service {
  private cache = new Map<string, App4Svc20CacheEntry<unknown>>();
  private requestQueue: Array<() => Promise<void>> = [];
  private processing = false;

  constructor(private config: App4Svc20ServiceConfig) {}
