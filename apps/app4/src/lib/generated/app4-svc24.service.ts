...
export interface App4Svc24ServiceConfig {
  baseUrl: string;
  apiKey: string;
  timeout: number;
...
export interface App4Svc24CacheEntry<T> {
  data: T;
  timestamp: number;
  ttl: number;
  key: string;
...
export class App4Svc24Service {
  private cache = new Map<string, App4Svc24CacheEntry<unknown>>();
  private requestQueue: Array<() => Promise<void>> = [];
  private processing = false;

  constructor(private config: App4Svc24ServiceConfig) {}
