...
export interface App2Svc13ServiceConfig {
  baseUrl: string;
  apiKey: string;
  timeout: number;
...
export interface App2Svc13CacheEntry<T> {
  data: T;
  timestamp: number;
  ttl: number;
  key: string;
...
export class App2Svc13Service {
  private cache = new Map<string, App2Svc13CacheEntry<unknown>>();
  private requestQueue: Array<() => Promise<void>> = [];
  private processing = false;

  constructor(private config: App2Svc13ServiceConfig) {}
