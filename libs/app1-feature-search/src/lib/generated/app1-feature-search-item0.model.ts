export enum App1FeatureSearchItem0Status {
  Active = 'active',
  Inactive = 'inactive',
  Pending = 'pending',
  Archived = 'archived',
  Draft = 'draft',
}

export enum App1FeatureSearchItem0Type {
  Default = 'default',
  Custom = 'custom',
  System = 'system',
  Template = 'template',
}

export interface App1FeatureSearchItem0Config {
  maxRetries: number;
  timeout: number;
  batchSize: number;
  enableCache: boolean;
  logLevel: 'debug' | 'info' | 'warn' | 'error';
}

export interface App1FeatureSearchItem0Metadata {
  version: number;
  lastModified: Date;
  author: string;
  checksum: string;
}

export interface IApp1FeatureSearchItem0 {
  id: string;
  name: number;
  label: boolean;
  value: Date;
  count: App1FeatureSearchItem0Status;
  enabled: App1FeatureSearchItem0Type;
  createdAt?: string;
  updatedAt?: number;
}

export interface IApp1FeatureSearchItem0ListResponse {
  items: IApp1FeatureSearchItem0[];
  total: number;
  page: number;
  pageSize: number;
  hasMore: boolean;
}

export interface IApp1FeatureSearchItem0Filter {
  query?: string;
  status?: App1FeatureSearchItem0Status[];
  type?: App1FeatureSearchItem0Type;
  dateFrom?: Date;
  dateTo?: Date;
  sortBy?: keyof IApp1FeatureSearchItem0;
  sortOrder?: 'asc' | 'desc';
}

export class App1FeatureSearchItem0Model implements IApp1FeatureSearchItem0 {
  id: string = undefined as any;
  name: number = undefined as any;
  label: boolean = undefined as any;
  value: Date = undefined as any;
  count: App1FeatureSearchItem0Status = undefined as any;
  enabled: App1FeatureSearchItem0Type = undefined as any;
  createdAt: string = undefined as any;
  updatedAt: number = undefined as any;

  constructor(data?: Partial<IApp1FeatureSearchItem0>) {
    if (data) Object.assign(this, data);
  }

  toJSON(): IApp1FeatureSearchItem0 {
    return { ...this } as IApp1FeatureSearchItem0;
  }

  clone(): App1FeatureSearchItem0Model {
    return new App1FeatureSearchItem0Model(this.toJSON());
  }

  validate(): string[] {
    const errors: string[] = [];
    if (!this.id) errors.push('id is required');
    if (!this.name) errors.push('name is required');
    return errors;
  }
}