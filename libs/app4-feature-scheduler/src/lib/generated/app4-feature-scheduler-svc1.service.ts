...
export class App4FeatureSchedulerSvc1Service {
  cache = new Map<string, App4FeatureSchedulerSvc1CacheEntry<unknown>>();
  requestQueue: Array<() => Promise<void>> = [];
  processing = false;
  config: App4FeatureSchedulerSvc1ServiceConfig;

  constructor(config: App4FeatureSchedulerSvc1ServiceConfig) {
    this.config = config;
  }
