...
export class SharedValidationSvc5Service {
  cache = new Map<string, SharedValidationSvc5CacheEntry<unknown>>();
  requestQueue: Array<() => Promise<void>> = [];
  processing = false;
  config: SharedValidationSvc5ServiceConfig;

  constructor(config: SharedValidationSvc5ServiceConfig) {
    this.config = config;
  }
