export enum App3FeatureTemplatesItem0Status {
  Active = 'active',
  Inactive = 'inactive',
  Pending = 'pending',
  Archived = 'archived',
  Draft = 'draft',
}

export enum App3FeatureTemplatesItem0Type {
  Default = 'default',
  Custom = 'custom',
  System = 'system',
  Template = 'template',
}

export interface App3FeatureTemplatesItem0Config {
  maxRetries: number;
  timeout: number;
  batchSize: number;
  enableCache: boolean;
  logLevel: 'debug' | 'info' | 'warn' | 'error';
}

export interface App3FeatureTemplatesItem0Metadata {
  version: number;
  lastModified: Date;
  author: string;
  checksum: string;
}

export interface IApp3FeatureTemplatesItem0 {
  id: string;
  name: number;
  label: boolean;
  value: Date;
  count: App3FeatureTemplatesItem0Status;
  enabled: App3FeatureTemplatesItem0Type;
  createdAt?: string;
  updatedAt?: number;
}

export interface IApp3FeatureTemplatesItem0ListResponse {
  items: IApp3FeatureTemplatesItem0[];
  total: number;
  page: number;
  pageSize: number;
  hasMore: boolean;
}

export interface IApp3FeatureTemplatesItem0Filter {
  query?: string;
  status?: App3FeatureTemplatesItem0Status[];
  type?: App3FeatureTemplatesItem0Type;
  dateFrom?: Date;
  dateTo?: Date;
  sortBy?: keyof IApp3FeatureTemplatesItem0;
  sortOrder?: 'asc' | 'desc';
}

export class App3FeatureTemplatesItem0Model implements IApp3FeatureTemplatesItem0 {
  id: string = undefined as any;
  name: number = undefined as any;
  label: boolean = undefined as any;
  value: Date = undefined as any;
  count: App3FeatureTemplatesItem0Status = undefined as any;
  enabled: App3FeatureTemplatesItem0Type = undefined as any;
  createdAt: string = undefined as any;
  updatedAt: number = undefined as any;

  constructor(data?: Partial<IApp3FeatureTemplatesItem0>) {
    if (data) Object.assign(this, data);
  }

  toJSON(): IApp3FeatureTemplatesItem0 {
    return { ...this } as IApp3FeatureTemplatesItem0;
  }

  clone(): App3FeatureTemplatesItem0Model {
    return new App3FeatureTemplatesItem0Model(this.toJSON());
  }

  validate(): string[] {
    const errors: string[] = [];
    if (!this.id) errors.push('id is required');
    if (!this.name) errors.push('name is required');
    return errors;
  }
}